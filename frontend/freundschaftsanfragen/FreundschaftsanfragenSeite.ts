...
const fill2 = (data: any) => {
    ...
    for (const user of data.eingehend) {
          ...
          li.innerHTML = `<b>${user.eingehendeAnfragen.name}</b>&nbsp;&nbsp;&nbsp;<form action="/api/freundesliste/web/akzeptiereanfrage" method="post" style="display:inline-block"><input name="uuid" value="${user.von}" style="display: none;"><button><img class="icon small" src="/assets/user-check-svgrepo-com.svg" alt="akzeptieren" /></button></form>&nbsp;&nbsp;&nbsp;<form action="/api/freundesliste/web/ablehnenanfrage" method="post" style="display:inline-block"><input name="uuid" value="${user.von}" style="display: none;"><button><img class="icon small" src="/assets/user-x-svgrepo-com.svg" alt="ablehnen" /></button></form>`;
          eingehend?.appendChild(li);
    }
    for (const user of data.ausgehend) {
          const li = document.createElement("li");
          li.innerHTML = `<b>${user.ausgehendeAnfragen.name}</b>&nbsp;&nbsp;&nbsp;<form action="/api/freundesliste/web/abortanfrage" method="post" style="display:inline-block"><input name="uuid" value="${user.zu}" style="display: none;"><button><img class="icon small" src="/assets/user-x-svgrepo-com.svg" alt="zurückziehen" /></button></form>`
