...
async function createInviteCode(code = "X") {
    await request(app).post("/api/registrierungscodes/web/addInviteCode").send({ code, passwort: "Das Adminpasswort" });
}

async function registerAndLogin(username: string) {
    await createInviteCode();
    await request(app).post("/api/nutzer/web/registrieren").send({ username, passwort: username, code: "X" });
    const login = await request(app).post("/api/nutzer/web/anmelden").send({ username, passwort: username });
    return login.headers["set-cookie"];
}

async function loginAsAdmin(username: string) {
    const cookie = await registerAndLogin(username);
    await request(app).post("/api/nutzer/web/elevate").set("Cookie", cookie).send({ passwort: "Das Adminpasswort" });
    return cookie;
}

async function addTestBaureihe(cookie: any, ubid = "a") {
    await request(app).post("/api/baureihe/web/add").set("Cookie", cookie).send({ ubid, name: "b", beschreibung: "c" });
}
...
test("Baureihe als gefunden markieren API", async () => {
    ...
    const response = await request(app).post("/api/aktivitaet/web/setgefunden").set("Cookie", cookie).send({ ubid: "a" });
    expect(response.status).toBe(302);
    expect(response.headers.location).toContain("/home?successMessage");
});

test("addInviteCode API", async () => {
    const response = await request(app).post("/api/registrierungscodes/web/addInviteCode").send({ code: "TESTCODE", passwort: "Das Adminpasswort" });
    expect(response.status).toBe(302);
...
test("Account registrieren API", async () => {
    await createInviteCode();
    const response = await request(app).post("/api/nutzer/web/registrieren").send({ username: "abc", passwort: "abc", code: "X" });
    expect(response.status).toBe(302);
...
test("Account anmelden API", async () => {
    await createInviteCode();
    await request(app).post("/api/nutzer/web/registrieren").send({ username: "def", passwort: "def", code: "X" });
    const response = await request(app).post("/api/nutzer/web/anmelden").send({ username: "def", passwort: "def" });
    expect(response.status).toBe(302);
...
test("Freund hinzufügen API", async () => {
    ...
    const response = await request(app).post("/api/freundesliste/web/add").set("Cookie", cookieA).send({ uuid: uuidB });
...
test("Freund entfernen API", async () => {
    ...
    await request(app).post("/api/freundesliste/web/add").set("Cookie", cookieD).send({ uuid: uuidE });
    const removeResponse = await request(app).post("/api/freundesliste/web/remove").set("Cookie", cookieD).send({ uuid: uuidE });
...
test("Baureihen von Freunden abrufen API", async () => {
    ...
    await request(app).post("/api/freundesliste/web/add").set("Cookie", cookieA).send({ uuid: uuidB });
    await request(app).post("/api/freundesliste/web/akzeptiereanfrage").set("Cookie", cookieB).send({ uuid: uuidA });
    await request(app).post("/api/aktivitaet/web/setgefunden").set("Cookie", cookieB).send({ ubid: "a" });
...
test("Gefundene Baureihen API", async () => {
    const cookie = await loginAsAdmin("F");
    await addTestBaureihe(cookie);
    await request(app).post("/api/aktivitaet/web/setgefunden").set("Cookie", cookie).send({ ubid: "a" });
...
test("Baureihe hinzufügen API", async () => {
    const cookie = await loginAsAdmin("F");
    const response = await request(app).post("/api/baureihe/web/add").set("Cookie", cookie).send({ ubid: "ab", name: "bb", beschreibung: "cb" });
    expect(response.status).toBe(302);
});
